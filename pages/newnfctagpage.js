	...
	getThisUsersAccounts( function (thisUsersAccounts) {
		...
		config.db.get("/" + "profile," + config.user.name , function (error, profile){
			...
			thisUsersAccounts.rows.forEach( function (trading_name) { 
				console.log( "Users Trading Names" + JSON.stringify( trading_name ) );

				//check profile mode if merchant add all trading names.
				if (typeof profile.mode != 'undefined' && profile.mode === true) {
					thisTag.trading_names.push( { "trading_name":trading_name.key.trading_name, "currency":trading_name.key.currency, "active": false } )
				} else {
					thisTag.trading_names.push( { "trading_name":trading_name.key.trading_name, "currency":trading_name.key.currency, "active": true } )
				}
	        ...
	        $( "#content form" ).off("submit").submit( function(e) {
				...
				if (window.nfc == undefined) {
					alert("NFC plugin not loaded");
				} else {
					window.nfc.removeMimeTypeListener("application/com.openmoney.mobile", window.nfcListner, function () {
						// success callback
					}, function () {
						// failure callback
					});
					...
					var newTagListner = function (nfcEvent) {
						...
						if (!mutableLock) {
							...
							if (tag.isWritable) {
									...
									getTradingNames(thisUsersAccounts, doc, function (error, trading_names) {
										if (error) {
											...
											nfc.write(message, function () {
												...
												navigator.notification.alert("Successfully written to NFC Tag!", function () {
													goManageNFC([])
												}, "Success", "OK")
											}, function () {
												...
												navigator.notification.alert("Failed to write to NFC Tag!", function () {
												}, "Failed", "OK")
											});
										}
									});
								...
								navigator.notification.alert("Tag is not writeable!", function () {
								}, "Not Writeable", "OK")
							}
						}
					};

					window.nfc.addNdefListener(newTagListner, function () { // success callback
						//alert( "Waiting for NFC tag" );
						navigator.notification.alert("Waiting for NFC tag", function () {
						}, "Waiting", "OK")
						...
						if (error == "NFC_DISABLED") {
							...
							navigator.notification.alert("Error adding NDEF listener:" + JSON.stringify(error), function () {
							}, "Error", "OK")
						}
